...
static const float c_sqrtD3 = 3.0 / C*C;                                // 3/(c^2)
static const float c_2sqrtSqrtD9 = 9.0 / (2.0*C*C*C*C);                 // 9/(2*(c^4))
static const float c_2squrD3 = 3.0 / (2.0*C*C);                         // 3/(2*(c^2))
...
float __AP_D2Q9_PRIVATE_F(const float omega, const float data_fi, const float feq);
...
/** 使用1234方向像素值更新当前像素的速度
   ...
   @return 更新了的该像素数据
*/
float4 ap_d2q9_updateDataF1234(const AP_D2Q9_Fi data1_r, const AP_D2Q9_Fi data2_u, const AP_D2Q9_Fi data3_l, const AP_D2Q9_Fi data4_d,
    const float alpha, const float omega)
{
    // 用我右边像素的fpre和feq值来更新我的fpre值
    float f3 = __AP_D2Q9_PRIVATE_UpdateFi(data1_r, float2(-1,  0), data1_r.f1234.z, weight1234, alpha, omega);

    // 用我上边像素的fpre和feq值来更新我的fpre值
    float f4 = __AP_D2Q9_PRIVATE_UpdateFi(data2_u, float2( 0, -1), data2_u.f1234.w, weight1234, alpha, omega);

    // 用我左边像素的fpre和feq值来更新我的fpre值
    float f1 = __AP_D2Q9_PRIVATE_UpdateFi(data3_l, float2( 1,  0), data3_l.f1234.x, weight1234, alpha, omega);

    // 用我下边像素的fpre和feq值来更新我的fpre值
    float f2 = __AP_D2Q9_PRIVATE_UpdateFi(data4_d, float2( 0,  1), data4_d.f1234.y, weight1234, alpha, omega);
...
/** 使用5678像素值更新当前像素的速度
   ...
   @return 更新了的该像素数据
*/
float4 ap_d2q9_updateDataF5678(const AP_D2Q9_Fi data5_ru, const AP_D2Q9_Fi data6_lu, const AP_D2Q9_Fi data7_ld, const AP_D2Q9_Fi data8_rd,
    const float alpha, const float omega)
{
    // 用我右上边像素的fpre和feq值来更新我的fpre值
    float f7 = __AP_D2Q9_PRIVATE_UpdateFi(data5_ru, float2(-sqrt2by2, -sqrt2by2), data5_ru.f5678.z, weight5678, alpha, omega);

    // 用我左上边像素的fpre和feq值来更新我的fpre值
    float f8 = __AP_D2Q9_PRIVATE_UpdateFi(data6_lu, float2( sqrt2by2, -sqrt2by2), data6_lu.f5678.w, weight5678, alpha, omega);

    // 用我左下边像素的fpre和feq值来更新我的fpre值
    float f5 = __AP_D2Q9_PRIVATE_UpdateFi(data7_ld, float2( sqrt2by2,  sqrt2by2), data7_ld.f5678.x, weight5678, alpha, omega);

    // 用我右下边像素的fpre和feq值来更新我的fpre值
    float f6 = __AP_D2Q9_PRIVATE_UpdateFi(data8_rd, float2(-sqrt2by2,  sqrt2by2), data8_rd.f5678.y, weight5678, alpha, omega);
...
float __AP_D2Q9_PRIVATE_UpdateFi(const AP_D2Q9_Fi data, const float2 dir, const float data_fi, const float weight, const float alpha, const float omega)
{
    ...
    float feq = __AP_D2Q9_PRIVATE_FEQ(weight, rho, alpha, v, dir);
...
float __AP_D2Q9_PRIVATE_F(const float omega, const float data_fi, const float feq)
{
    return (1 - omega)*data_fi + omega*feq;
}
